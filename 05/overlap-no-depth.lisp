(in-package :gl-tutorial.5.overlap-no-depth)
...
(defun initialize-vertex-array-objects ()
  (setf *vao-1* (gl:gen-vertex-array))
  (gl:bind-vertex-array *vao-1*)
  ...
  (let ((color-data-offset (* (cffi:foreign-type-size :float) 3 +num-of-vertices+)))
    ...
    (setf *vao-2* (gl:gen-vertex-array))
    (gl:bind-vertex-array *vao-2*)

    (let ((pos-data-offset (* (cffi:foreign-type-size :float) 3 (/ +num-of-vertices+ 2))))
      (incf color-data-offset (* (cffi:foreign-type-size :float) 4 (/ +num-of-vertices+ 2)))
      (gl:enable-vertex-attrib-array 0)
      (gl:enable-vertex-attrib-array 1)
      (gl:vertex-attrib-pointer 0 3 :float :false 0 pos-data-offset)
      (gl:vertex-attrib-pointer 1 4 :float :false 0 color-data-offset)
      (gl:bind-buffer :element-array-buffer *index-buffer-object*)))
  (gl:bind-vertex-array 0))
...
(defmethod render ((window main-window))
  ...
  (gl:bind-vertex-array *vao-1*)
  (uniform :vec :offset (vec3 0.0 0.0 0.0))
